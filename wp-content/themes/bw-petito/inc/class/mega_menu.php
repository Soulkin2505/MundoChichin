...
if($bzotech_config['mega_menu'] == '1'){
    class Bzotech_CustomMenu
    {
        static $allFields;
        static function init()
        {
            self::$allFields=array(
                'enable_megamenu'=>array(
                    'label'=>esc_html__('Enable Mega menu','bw-petito'),
                    'type'=>'checkbox',
                    'depth'=>0,
                ),
                'icon_menu2'=>array(
                    'label'=>esc_html__('Custom width Mega menu. Example "500px".','bw-petito'),
                    'type'=>'text',
                ),
                
                'image'=>array(
                    'label'=>esc_html__('Image Preview','bw-petito'),
                    'type'=>'image',
                ),
                'content2'=>array(
                    'label'=>esc_html__('Content From Mega Item','bw-petito'),
                    'type'=>'select',
                    'depth'=>0,
                    'choices'=> bzotech_list_post_type('bzotech_mega_item',true)
                ),
        ...
        static function _field_type_image($item,$key,$value,$d=0){
            ...
            <p class="field-custom description description-wide">
                ...
                <div class="wrap-metabox wrap-live-previews">
                    <div class="live-previews">
                        <?php if(!empty($item_value)):?>
                            <img src="<?php echo wp_get_attachment_url($item_value)?>"/>
                        <?php endif;?>
                    </div>
                    <a class="button button-primary sv-button-remove"> <?php esc_html_e("Remove",'bw-petito')?></a>
                    <a class="button button-primary sv-button-upload-id"><?php esc_html_e("Upload",'bw-petito')?></a>
                    <input id="<?php echo esc_attr($item_id.$key) ?>" name="<?php echo esc_attr($key.'['. $item_id.']'); ?>" type="hidden" class="widefat code edit-menu-item-custom sv-image-value" value="<?php echo esc_attr($item_value)?>"/>
                </div>
            </p>
            <?php
        }
    ...
    Bzotech_CustomMenu::init();
    class Bzotech_WalkerNavMenu extends Walker_Nav_Menu
    {
        /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( ! empty( $item->_invalid ) ) {
                ...
                $title = sprintf( esc_html__( '%s (Invalid)' ,'bw-petito'), $item->title );
            } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
                $classes[] = 'pending';
                /* translators: %s: title of menu item in draft status */
                $title = sprintf( esc_html__('%s (Pending)','bw-petito'), $item->title );
            }
            ...
            <dl class="menu-item-bar">
                <dt class="menu-item-handle">
                    <span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo bzotech_add_html_attr($submenu_text); ?>><?php esc_html_e( 'sub item' ,'bw-petito'); ?></span></span>
                        <span class="item-controls">
                            ...
                            <span class="item-order hide-if-js">
                                ...
                                ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','bw-petito'); ?>">&#8593;</abbr></a>
                                |
                                <a href="<?php
                                echo wp_nonce_url(
                                    add_query_arg(
                                ...
                                ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','bw-petito'); ?>">&#8595;</abbr></a>
                            </span>
                            <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item','bw-petito'); ?>" href="<?php
                            ...
                            ?>"><?php esc_html_e( 'Edit Menu Item' ,'bw-petito'); ?></a>
                        </span>
                </dt>
            </dl>
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                <?php if( 'custom' == $item->type ) : ?>
                    <p class="field-url description description-wide">
                        <label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'URL' ,'bw-petito'); ?><br />
                ...
                <p class="description description-thin">
                    <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Navigation Label' ,'bw-petito'); ?><br />
                        <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                    </label>
                </p>
                <p class="description description-thin">
                    <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Title Attribute' ,'bw-petito'); ?><br />
                ...
                <p class="field-link-target description">
                    <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                        ...
                        <?php esc_html_e( 'Open link in a new window/tab','bw-petito' ); ?>
                    </label>
                </p>
                <?php

                Bzotech_CustomMenu::AdminAddFields($item,$depth);

                ?>
                <p class="field-css-classes description description-thin">
                    <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'CSS Classes (optional)' ,'bw-petito'); ?><br />
                ...
                <p class="field-xfn description description-thin">
                    <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Link Relationship (XFN)' ,'bw-petito'); ?><br />
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'Description' ,'bw-petito'); ?><br />
                        <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                        <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.' ,'bw-petito'); ?></span>
                    </label>
                </p>

                <p class="field-move hide-if-no-js description description-wide">
                    <label>
                        <span><?php esc_html_e( 'Move' ,'bw-petito'); ?></span>
                        <a href="#" class="menus-move-up"><?php esc_html_e( 'Up one' ,'bw-petito'); ?></a>
                        <a href="#" class="menus-move-down"><?php esc_html_e( 'Down one' ,'bw-petito' ); ?></a>
                        <a href="#" class="menus-move-left"></a>
                        <a href="#" class="menus-move-right"></a>
                        <a href="#" class="menus-move-top"><?php esc_html_e( 'To the top' ,'bw-petito'); ?></a>
                    </label>
                </p>
                ...
                <div class="menu-item-actions description-wide submitbox">
                    <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                        <p class="link-to-original">
                            <?php printf( esc_html__('Original: %s','bw-petito'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    ); ?>"><?php esc_html_e( 'Remove' ,'bw-petito'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
                    ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel','bw-petito'); ?></a>
                </div>
