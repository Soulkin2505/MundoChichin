        ...
        static function _do_import()
        {
            if(isset($_REQUEST['bzotech_do_import']) and $_REQUEST['bzotech_do_import'] || isset($_REQUEST['media']))
            {
                ...
                if(!current_user_can('manage_options')){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('You do not have permission to do this','bw-petito')));die;
                }
                self::load_lib();

                //Check Importer Plugins was installed
                if ( !class_exists( 'WP_Importer' ) or ! class_exists( 'WP_Import' ) ){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('Importer Class Was Not Installed','bw-petito')));die;
                }
                ...
                $data_dir = $data_dir_root.'/demo'.$demo;
                if($step==1) {
                    ...
                    if(!empty($data_option)){
                        ...
                        if(!function_exists('bzotech_get_option_name'))
                        {
                            echo json_encode( array(
                                    'status'   =>0,
                                    'messenger'=>"<span class='red'>".esc_html__("Plugin: Bzotech core must be installed first. Stop working!",'bw-petito')."</span>",
                                    'next_url' => ''
                                )
                            );
                            die;
                        ...
                        echo json_encode( array(
                                'status'   =>"ok",
                                'messenger'=>esc_html__("Importing the demo theme options...",'bw-petito')."<span>".esc_html__("DONE!",'bw-petito')."</span><br>",
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("Theme option contain NULL content. Stop working!",'bw-petito')."</span>",
                                'next_url' => ''
                            )
                        );
                    }
                ...
                if($step==2){
                    ...
                    if(!empty($data_object)){
                        ...
                        echo json_encode( array(
                                'status'   =>1,
                                'messenger'=>esc_html__("Importing the demo widgets...",'bw-petito')." <span>".esc_html__("DONE!",'bw-petito')."</span>.<br>",
                                'next_url' => admin_url(self::$_import_page."&bzotech_do_import={$demo}&media={$import_media}&package={$package}&step=" . ($step + 1)),
                            )
                        );
                    }
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("Widget option contain NULL content. Stop working!",'bw-petito')."</span>",
                                'next_url' => ''
                            )
                        );
                    }
                ...
                if($step==3){
                    ...
                    if(!$file_name){
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("File Not Found. Stop working!",'bw-petito')."</span>",
                                'next_url' => ''
                            )
                        );
                        die;
                ...
                if($step==4) {
                    ...
                    echo json_encode(array(
                            'status' => 1,
                            'messenger' => esc_html__("Importing menu settings ...",'bw-petito')." <span>".esc_html__("DONE!",'bw-petito')."</span><br>",
                            'next_url' => $nexturl,
                        )
                    );
                }
                if($step==5) { 
                    $imported_data = array();
                    $site_settings = Utils::read_json_file($data_dir_root.'/site-settings.json');
                    ...
                    echo json_encode(array(
                            'status' => 1,
                            'messenger' => esc_html__("Importing elementor settings ...",'bw-petito')." <span>".esc_html__("DONE!",'bw-petito')."</span><br>",
                            'next_url' => $nexturl,
                        )
                    );
                }
                ...
                if($step == 6){
                    ...
                    if($stt_file<count($ds_file) && class_exists('RevSlider')){   
                        ...
                        if(!$set_woocommerce_page) {
                            $next_url = '';
                            $messenger = '<span>'.esc_html__("All Done! Have Fun",'bw-petito').'</span>';
                        }
                        else {
                        ...
                        echo json_encode( array(
                                'status'   =>"ok",
                                'messenger'=>esc_html__("Setting reading options...",'bw-petito')." <span>".esc_html__("DONE!",'bw-petito')."</span><br/>".$messenger,
                                'next_url' => $next_url,
                            )
                        );
                    }
                ...
                if($step==7){
                    ...
                    echo json_encode( array(
                            'status'   =>"ok",
                            'messenger'=>esc_html__('Setting WooCommerce options...','bw-petito').' <span>'.esc_html__('DONE!','bw-petito').'</span><br/><span>'.esc_html__('All Done! Have Fun','bw-petito').'</span><br/><a class="close-import bt-done" href="#">Close</a><a class="bt-done bt-next" href="https://bzotech.com/medxtore-wordpress-theme-documentation/#after-import"  target="_blank">Next Step<span class=" dashicons-before dashicons-arrow-right-alt2"></span></a><a class="bt-done" href="'.get_admin_url().'admin.php?page=_options&tab=1">Theme Options</a><a class="bt-done" href="'.home_url().'">Visit Site</a>',
                            'next_url' => '',
                        )
                    );
                }
