...
add_action( 'comment_form_before', 'bzotech_comment_form_before');
if(!function_exists('bzotech_comment_form_before')){
    function bzotech_comment_form_before(){
        $fields = apply_filters( 
            'wpse_comment_fields', 
            array( 'comment', 'author', 'url', 'email', 'submit' )
        );
        $o = new Bzotech_Rearrange_Comment_Fields;
        $o->set_fields( $fields )->init();
    }
}
class Bzotech_Rearrange_Comment_Fields
{
    private $html       = array();
...
add_filter( 'wpse_comment_fields', 'bzotech_comment_fields');
if(!function_exists('bzotech_comment_fields')){
    function bzotech_comment_fields($fields){
        $fields = array(  'author', 'email', 'comment', 'submit' );
        return $fields;
    }
}