...
if(!class_exists('Bzotech_Attribute_Filter') && class_exists("woocommerce")){
    class Bzotech_Attribute_Filter extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct( false, esc_html__('BZOTECH Attribute Filter','bw-petito'),
                array( 'description' => esc_html__( 'Filter product shop page', 'bw-petito' ), ));
                ...
                if(!empty($terms)){
                    ...
                    switch ($attr->attribute_type){
                                    ...
                                    foreach ($terms as $term){
                                        ...
                                        if(is_object($term)){
                                            ...
                                            if(!empty($color)){
                                                ...
                                                echo sprintf(
                                                    '<a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-color '.$class_white_color.' swatch-%s %s"  title="%s" href="%s"><span class="span-trong" '.bzotech_add_html_attr('background-color:'.$color).'></span></a>',
                                                    esc_attr($attribute),
                                                    esc_attr( $term->slug ),
                                                    esc_attr( $term->slug ),
                                                    $selected,
                        ...
                        case 'label': ?>
                                ...
                                if(is_array($terms)){
                                    foreach ($terms as $term){
                                        if(is_object($term)){
                                            ...
                                            if(!empty($label)){
                                                echo sprintf(
                                                    '<a data-attribute="%s" data-term="%s" class="load-shop-ajax swatch swatch-label swatch-%s %s" title="%s" data-value="%s" href="%s"><span class="label-attr">%s</span><span class="desc">%s</span></a>',
                                                    esc_attr($attribute),
                                                    esc_attr( $term->slug ),
                                                    esc_attr( $term->slug ),
                                                    $selected,
                            ...
                            break;
                        default :
                            echo    '<ul class="list-filter  attribute-type-default">';
                            if(is_array($terms)){
                                foreach ($terms as $term) {
                                    if(is_object($term)){
                                        if(in_array($term->slug, $term_current)) $active = 'active';
                                        else $active = '';
                                        echo    '<li class="font-title color222 title16  font-medium '.esc_attr($term->slug).'-inline '.esc_attr($active).'">
                                                    <a data-attribute="'.esc_attr($attribute).'" data-term="'.esc_attr($term->slug).'" class="color222 load-shop-ajax bgcolor-'.esc_attr($term->slug).'" href="'.esc_url(bzotech_get_filter_url('filter_'.$attribute,$term->slug)).'">
                                                    <span></span>'.$term->name.'
                                                    </a>
        ...
        function form( $instance ) {
            ...
            <p>
                <label><?php esc_html_e( 'Attribute:' ,'bw-petito'); ?></label></br>
                <select id="<?php echo esc_attr($this->get_field_id( 'attribute' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'attribute' )); ?>">
                    ...
                    else echo esc_html__("No any attribute.",'bw-petito');
                    ?>
                </select>
            </p>
            <?php
