...
if(!class_exists('Bzotech_Widget_Product_Slider')) {
    class Bzotech_Widget_Product_Slider extends WC_Widget
    {
        ...
        public function __construct()
        {
            ...
            $this->widget_description = esc_html__('Display a list of your products on your site.', 'bw-petito');
            $this->widget_id = 'Bzotech_Widget_Product_Slider';
            $this->widget_name = esc_html__('BZOTECH Products slider', 'bw-petito');
        ...
        public function init_settings()
        {
            ...
            $this->settings = array_merge(
                array(
                    'title'  => array(
                        'type'  => 'text',
                        'std'   => esc_html__( 'Products list', 'bw-petito' ),
                        'label' => esc_html__( 'Title', 'bw-petito' ),
                    ),
                    'number_post'  => array(
                        'type'  => 'text',
                        'std'   => '8',
                        'label' => esc_html__( 'Number post (Default: 8)', 'bw-petito' ),
                    ),
                    'product_type' => array(
                        'type'  => 'select',
                        'std'   => '',
                    ...
                    'title_category' => array(
                        ...
                        'label' => esc_html__( 'Filter by category (Default get all)', 'bw-petito' ),
                    ),
                ...
                array(
                    'order_by' => array(
                        ...
                        'options' => array(
                            'none'   => esc_html__('None','bw-petito'),
                            'ID'  => esc_html__('Post ID','bw-petito'),
                            'author' => esc_html__('Author','bw-petito'),
                            'title' => esc_html__('Post Title','bw-petito'),
                            'name' => esc_html__('Post Name','bw-petito'),
                            'date' => esc_html__('Post Date','bw-petito'),
                            'modified' => esc_html__('Last Modified Date','bw-petito'),
                            'parent' => esc_html__('Post Parent','bw-petito'),
                            'rand' => esc_html__('Random','bw-petito'),
                            'comment_count' => esc_html__('Comment Count','bw-petito'),
                            'post_views' => esc_html__('View Post','bw-petito'),
                            'rating' => esc_html__('Rating Product','bw-petito'),
                            'price' => esc_html__('Sort by price','bw-petito'),
                        ),
                    ),
                    'order' => array(
                        'type'  => 'select',
                        ...
                        'options' => array(
                            'DESC' => esc_html__( 'Descending', 'bw-petito' ),
                            'ASC'  => esc_html__( 'Ascending', 'bw-petito' ),
                        ),
                    ),
                    ...
                    'number_row' => array(
                        'type'  => 'text',
                        'std'   => '4',
                        'label' => esc_html__( 'Number product in item silder (Default: 4)', 'bw-petito' ),
                    ),
                    'image_size' => array(
                        'type'  => 'text',
                        'std'   => '',
                        'label' => esc_html__( 'Custom image size (Example: "thumbnail", "medium", "large", "full" or other sizes defined by theme. Alternatively enter size in pixels : 200x100 (Width x Height))', 'bw-petito' ),
                    ),
                )
            );
        }
        ...
        public function widget($args, $instance)
        {
            ...
            echo bzotech_get_template_widget('widgets/product',false,array(
                'query'=>$query,
