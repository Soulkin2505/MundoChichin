...
$column         = bzotech_get_option('post_grid_column',3);
$size           = bzotech_get_option('post_grid_size');
$size_list      = bzotech_get_option('post_list_size');
$number         = get_option('posts_per_page');
$show_number    = bzotech_get_option('blog_number_filter');
$show_type     = bzotech_get_option('blog_type_filter');
...
$view = $type_active;
$item_wrap = 'class="list-col-item list-'.esc_attr($column).'-item list-2-item-tablet list-1-item-mobile item-grid-post-'.$item_style.'"';
$item_inner = 'class="item-post"';
$button_icon_pos = $button_icon = '';
$button_text = esc_html__("Read more", 'bw-petito');
$slug = $item_style;
...
<?php
bzotech_get_template('top-filter','',array('style'=>$type_active,'number'=>$number,'show_number'=>$show_number,'show_type'=>$show_type),true); 

$attr_product = array(
    'item_wrap'         => 'class="item-grid-product- list-3-item list-1-item-tablet list-1-item-mobile"',
                        ...
                        <div class="js-content-main list-post-wrap bzotech-row">
                            ...
                            <?php while (have_posts()) :the_post();?>
                                ...
                                if(get_post_type() == 'product'){
                                    echo '<div class="bzotech-col-md-12"><div class="item-post item-list-post-default">';
                                    bzotech_get_template_woocommerce('loop/grid/grid','',$attr_product,true);
                                    echo '</div></div>';
                                }else bzotech_get_template_post($view.'/'.$view,$slug,$attr,true);
                                $dem = $dem+1;
                                ?>
