...
<div <?php echo ''.$wdata->get_render_attribute_string('wrapper');?>>
	...
	<?php if(!empty($price)) echo '<h2 class="price-pricing-table">'.$price.'</h2>'; ?>
	<?php
			...
			<?php foreach ($list_pricing_table as $key => $value) {
				...
				if(!empty($value['icon_image']['url'])){
					...
					if($value['icon']['library'] == 'svg')
						$html_icon .= '<img alt="'.esc_attr__('svg','bw-petito').'" src ="'.esc_url($value['icon']['value']['url']).'">';
					else
	...
	<?php if(!empty($button_text)) {
		...
		echo '<div class="button-pricing-wapper"><a '.$wdata->get_render_attribute_string('button-inner').'>'.$button_text.'</a></div>';
	} ?>
</div>
