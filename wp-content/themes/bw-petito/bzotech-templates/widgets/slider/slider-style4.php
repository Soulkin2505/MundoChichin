...
<?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper' ).'>';?>
    <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper-slider' ).'>';?>
        <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-inner' ).'>';?>
            <?php 
            foreach (  $list_cate_2 as $key => $item ) {
                ...
                echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-item' ).'><div class="item-slider-'.$style.'">';
                    ...
                    if(!empty($item['template'])) 
                       ...
                       echo '<div class="flex-wrapper flex_direction-column align_items-center"><a class="adv-thumb-link-cate" '.$link_html.'>'.$image_html.'</a>
        <div class="info"><div class="info-cate"><h2 class="title font-title color-title font-medium title18"><a '.$link_html.'>'.$title_html.'</a></h2><div class="">'.$count_html.'</div></div></div></div>';
                    }  
