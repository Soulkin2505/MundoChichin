    ...
    <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-wrapper-slider' ).'>';?>
        <?php echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-inner' ).'>';?>
            ...
            foreach (  $list_testimonial2 as $key => $item ) {
                if($count % 2 == 0) $even_num='even_num';
                else $even_num='';
                $count++;
                $wdata->add_render_attribute('elbzotech-item', 'class', 'wslider-item elementor-repeater-item-'.$item['_id']);
                echo '<div '.$wdata->get_render_attribute_string( 'elbzotech-item' ).'><div class="item-slider-'.$style.' '.$even_num.'">';
                    
                    echo '<div class="flex-wrapper-column">';
                        echo '<div class="content-wrap">';
                           if(!empty($item['content'])) echo  '<div class="content-slider-custom box-content-custom">'.bzotech_parse_text_editor( $item['content']).'</div>';
                        echo '</div>';
                        if(!empty($item['image']['url'])) {
                            ...
                            echo '<div class="info-wrap"><a '.$wdata->get_render_attribute_string('img-link').' class="img-wrap">';
                            echo Group_Control_Image_Size::get_attachment_image_html( $list_testimonial2[$key], 'thumbnail', 'image' );
                            echo '</a>';
                            if(!empty($item['title'])) echo '<h3 class="font-title title20 font-semibold">'.$item['title'].'</h3>';
                            if(!empty($item['description'])) echo '<p class="desc title14">'.$item['description'].'</p>';
