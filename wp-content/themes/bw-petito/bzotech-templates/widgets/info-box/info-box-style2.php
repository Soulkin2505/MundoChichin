...
<div <?php echo apply_filters('bzotech_output_content', $wdata->get_render_attribute_string('wrapper'));?>>
			...
			echo'<div class="info-box-content">';
				if(!empty($title)){
					...
					echo'<h3 class="item-title-e title24 font-bold font-title"><a '.$wdata->get_render_attribute_string( 'link_title' ).'>'.$title.'</a></h3>';
				}
				if(!empty($sub_title)){
					echo'<p class="item-sub-title-e font-regular title18">'.$sub_title.'</p>';
				}
			echo'</div>';
			?>
</div>