...
<?php
	...
	function process_menu_item( $item ) {
		...
		if(isset($enable_megamenu)) $args['enable_megamenu'] = $enable_megamenu;
		if(isset($icon_menu2)) $args['icon_menu2'] = $icon_menu2;
		if(isset($content2)) $args['content2'] = $content2;
		if(isset($mega_menu_position)) $args['mega_menu_position'] = $mega_menu_position;
		if(isset($image)) $args['image'] = $image;

		$id = fix_wp_update_nav_menu_item( $menu_id, 0, $args );
...
function fix_wp_update_nav_menu_item( $menu_id = 0, $menu_item_db_id = 0, $menu_item_data = array() ) {
    $menu_id = (int) $menu_id;
    ...
    if ( 'custom' == $args['menu-item-type'] ) {
        ...
        $args['menu-item-url'] = str_replace($bzotech_config['import_config']['demo_url'], get_home_url('/'), $args['menu-item-url']);
    }
    ...
    //Mega metabox
    if(isset($args['enable_megamenu']))
    if($args['enable_megamenu']) {
    	update_post_meta( $menu_item_db_id, 'enable_megamenu', $args['enable_megamenu'] );
    	update_post_meta($menu_item_db_id,'enable_megamenu123',$args['enable_megamenu']);
    }
    if(isset($args['icon_menu2'])) update_post_meta( $menu_item_db_id, 'icon_menu2', $args['icon_menu2'] );
    if(isset($args['content2'])) update_post_meta( $menu_item_db_id, 'content2', $args['content2'] );
    if(isset($args['mega_menu_position'])) update_post_meta( $menu_item_db_id, 'mega_menu_position', $args['mega_menu_position'] );
    if(isset($args['image'])) update_post_meta( $menu_item_db_id, 'image', $args['image'] );
