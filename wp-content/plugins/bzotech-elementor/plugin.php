...
class Plugin {
	...
	private function includes() {	
		...
		$files2=glob($template_path."/inc/bzotech-elementor/*.php");
        ...
        if(!empty($files)){
            foreach ($files as $filename)
            {
            	...
            	if(!in_array($name, $names));{
            		$names[] = $name;
	            	$child_path = $stylesheet_path.'/inc/bzotech-elementor/'.$name.'.php';
	            	$theme_path = $template_path.'/inc/bzotech-elementor/'.$name.'.php';
